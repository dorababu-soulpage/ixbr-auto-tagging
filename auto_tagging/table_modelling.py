...
class NameMappingModel(pl.LightningModule):
    ...
    def __init__(self, labels, label2id, id2label):
        ...
        self.all_test_preds = []
        self.labels = labels
        self.finbert = AutoModelForSequenceClassification.from_pretrained(yaml_obj["IXBRL_MODELS"]["Tokenizer"],
...
modeleval = NameMappingModel.load_from_checkpoint(checkpoint_path="Models1\Table_inline_Model\SECtag_RarelabelModel-epoch=39-val_loss=0.26.ckpt",
                                                    labels=labels,
                                                    label2id = label2id,
                                                    id2label=id2label,
                                                    map_location= device,
