...
class Xbrl_Tag:
    ...
    def __init__(self):
        ...
        self.dei_tokenizer = AutoTokenizer.from_pretrained(
            yaml_obj["IXBRL_MODELS"]["Tokenizer"],
            add_prefix_space=True,
            do_lower_case=True,
        )

        self.dei_model = AutoModelForTokenClassification.from_pretrained(
            os.path.abspath(yaml_obj["IXBRL_MODELS"]["Dei_Model"])
        )
        self.dei_model = self.dei_model.to(self.device)

        self.notes_model = AutoModelForTokenClassification.from_pretrained(
            os.path.abspath(yaml_obj["IXBRL_MODELS"]["Notes_Model"])
        )
    ...
    def predict_dei_tags(self, total_rows):
        ...
        for input_row in total_rows:
            ...
            new_predicted_token_labels = [
                self.dei_model.config.id2label[t.item()] for t in new_predictions[0]
            ]
            decoded_string = self.dei_tokenizer.convert_ids_to_tokens(
                new_inputs["input_ids"][0]
            )
            reconstructed_row, reconstructed_predictions = post_process(
                decoded_string, new_predicted_token_labels
            )
    ...
    def predict_notes_tags(self, total_rows):
        ...
        for input_row in total_rows:
            ...
            new_predictions = torch.argmax(new_logits, dim=2)
            new_predicted_token_class = [
                self.notes_model.config.id2label[t.item()] for t in new_predictions[0]
            ]
            decoded_string = self.dei_tokenizer.convert_ids_to_tokens(
                new_inputs["input_ids"][0]
            )
